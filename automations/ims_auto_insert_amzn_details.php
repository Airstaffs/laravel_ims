...
$cronName = 'conversion_auto_import_logic';
$value = true;
...
$handler = loadAutomationHandler($Connect, $cronName);

if (!$handler) {
    die("No tblautomationshandler row found for: {$cronName}");
}

$handlerId = $handler['id'];
$cfg = $handler['config'];

$storeToRun = $cfg['store'] ?? 'RT';
$storeToRun = ($storeToRun === 'AR') ? 'AR' : 'RT'; // sanitize
...
$cfg['store'] = ($storeToRun === 'RT') ? 'AR' : 'RT';

updateAutomationHandler($Connect, $handlerId, $cfg);
    ...
    foreach ($allSkus as $skuperitem) {
        ...
        if (is_array($results) && isset($results['summaries']) && is_array($results['summaries'])) {
            foreach ($results['summaries'] as $summary) {
                ...
                } else if (isset($skucondition) && $skucondition == 'used_acceptable') {
                    $skucondition = "UsedAcceptable";
                } else {
...
*/
$successful = true;
function loadAutomationHandler(mysqli $Connect, string $cronName): ?array
{
    $sql = "SELECT id, additional_config
            FROM tblautomationshandler
            WHERE cron_automation = ?
            ORDER BY id DESC
            LIMIT 1";
    ...
    $cfg = json_decode($row['additional_config'] ?? '{}', true);
    if (!is_array($cfg))
        $cfg = [];
...
function updateAutomationHandler(mysqli $Connect, $id, array $cfg): void
{
    $json = json_encode($cfg, JSON_UNESCAPED_SLASHES);
    $sql = "UPDATE tblautomationshandler
            SET additional_config = ?, time_last_triggered = NOW()
            WHERE id = ?";
    $stmt = $Connect->prepare($sql);
...
function flipStore(string $current): string
{
    return ($current === 'RT') ? 'AR' : 'RT';
}
...
function updateCronInsertStatus($Connect, $MSKU, $sid)
{
    ...
    $stmt = mysqli_prepare($Connect, $sql);
    mysqli_stmt_bind_param($stmt, "ss", $MSKU, $sid);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
}
