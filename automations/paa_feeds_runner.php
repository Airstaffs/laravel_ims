<?php
/**
 * PAA Feeds Runner (Native PHP)
 *
 * What this version changes
 * --------------------------------------------------
 * - Keeps the SAME cron / trigger behavior as your current paa_runner.php
 * - Still snapshots original_price BEFORE any adjust
 * - Still restores original_price AFTER the rule window ends
 * - BUT instead of patching one SKU at a time for writes,
 *   it submits bulk price updates through JSON_LISTINGS_FEED
 *
 * Cached price behavior
 * --------------------------------------------------
 * - This version NO LONGER reads live Amazon price during adjust.
 * - It uses tblfnsku.amzn_item_price as the cached baseline price.
 * - If cached price is NULL, 0, or <= 0, the item is excluded from the run snapshot.
 * - The snapshot is frozen into tbl_paa_run_items.current_price/original_price.
 * - Restore uses tbl_paa_run_items.original_price, not tblfnsku, so later syncs do not break restore.
 *
 * Required app endpoint
 * --------------------------------------------------
 * This runner assumes your Laravel app exposes ONE internal endpoint:
 *
 ...
 *
 * Notes:
 * - The endpoint can internally do:
 *     createFeedDocument -> upload JSON -> createFeed -> poll until DONE/FATAL/CANCELLED
 * - If you want, I can give you that Laravel controller next.
 *
 * DB assumptions
 * --------------------------------------------------
 * Same tables as your current paa_runner.php, plus this cached field on tblfnsku:
 *
...
try {
    ...
    foreach ($automations as $automation) {
        ...
        if ($openRun) {
            ...
            if ($phase === 'restore') {
                process_restore_phase($mysqli, $automation, $openRun, $batchSize = $GLOBALS['BATCH_SIZE'], $maxAttempts = $GLOBALS['MAX_ATTEMPTS']);
                continue;
            }

            process_adjust_phase($mysqli, $automation, $openRun, $currentHHMM, $batchSize = $GLOBALS['BATCH_SIZE'], $maxAttempts = $GLOBALS['MAX_ATTEMPTS']);
            continue;
        }
        ...
        if ($existingWindowRun) {
            ...
            if (($existingWindowRun['phase'] ?? '') === 'restore') {
                process_restore_phase($mysqli, $automation, $existingWindowRun, $batchSize = $GLOBALS['BATCH_SIZE'], $maxAttempts = $GLOBALS['MAX_ATTEMPTS']);
            } else {
                process_adjust_phase($mysqli, $automation, $existingWindowRun, $currentHHMM, $batchSize = $GLOBALS['BATCH_SIZE'], $maxAttempts = $GLOBALS['MAX_ATTEMPTS']);
            }
        ...
        if ($newRun && ($newRun['phase'] ?? '') === 'adjust' && ($newRun['status'] ?? '') !== 'done') {
            process_adjust_phase($mysqli, $automation, $newRun, $currentHHMM, $batchSize = $GLOBALS['BATCH_SIZE'], $maxAttempts = $GLOBALS['MAX_ATTEMPTS']);
        }
    }
...
function get_snapshot_items_automatic($mysqli)
{
    ...
    while ($r = $res->fetch_assoc()) {
        ...
        $cachedPrice = isset($r['amzn_item_price']) ? (float) $r['amzn_item_price'] : null;

        if ($msku === '' || $storename === '' || $cachedPrice === null || $cachedPrice <= 0) {
            continue;
        }
...
function claim_restore_batch($mysqli, $runId, $batchSize, $maxAttempts)
{
    ...
    $sql = "
        SELECT id, msku, sku, storename, fnskuid, attempts, current_price, original_price, original_price
        FROM tbl_paa_run_items
        WHERE run_id=?
          AND adjusted_at_utc IS NOT NULL
          AND restored_at_utc IS NULL
            ...
            try {
                ...
                if ($currentPrice === null || $currentPrice <= 0) {
                    $stmt = $mysqli->prepare("
                        ...
                        WHERE id=?
                    ");
                    $err = 'No cached current price found in snapshot';
                    $stmt->bind_param('si', $err, $runItemId);
                    $stmt->execute();
                    $stmt->close();
