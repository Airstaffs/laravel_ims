...
function processRetrievedData($Connect, $retrievedData) {
    ...
    // Navigate to the return details correctly
    if (isset($xml->Message->return_details) && !empty($xml->Message->return_details)) {
        foreach ($xml->Message->return_details as $returnDetail) {
            ...
            $returnDetailsArray[] = $returnDetailArray;
        }
    } else {
        echo "<br>No return details found in the XML data.<br>";
    }

    // Create the final array (if needed)
    return [
        'message_type' => $messageType,
        'return_details' => $returnDetailsArray
    ];
}
...
function insertToDb($Connect, $response) {
    ...
    foreach ($response['return_details'] as $details) {
        ...
        if ($count > 0) {
            ...
            $stmt->execute();
            $stmt->close();
            echo "<br>Record inserted successfully: Order ID $amazonOrderId, ASIN $ASIN, MSKU $MSKU<br>";
        }
    }
}
