  ...
  <div class="p-2">
    <div class="d-flex align-items-center gap-2 mb-3">
      ...
      <button
        ...
        @click="$parent.hrContext.refreshRateHistory($parent.hrContext.rateHistoryFilterEmployeeId || null)"
      >
        <span
    ...
    <div class="table-responsive">
      <table class="table table-sm table-bordered align-middle">
        ...
        <tbody>
          <tr v-for="(row, i) in $parent.hrContext.filteredRateHistory" :key="row.id">
            <td>{{ i + 1 }}</td>
            <td>
              {{ row.employee_username || row.username || ('#' + row.employee_id) }}
            </td>
            <td>{{ row.effective_start }}</td>
            <td>{{ row.effective_end || 'Present' }}</td>
            <td>
              <span v-if="row.monthly_rate != null">₱{{ Number(row.monthly_rate).toFixed(2) }}</span>
              <span v-else>-</span>
            </td>
            <td>
            ...
            <td>{{ $parent.hrContext.formatDate(row.created_at) }}</td>
            <td>
              <span
