    ...
    <div class="sched-container">
        ...
        <section v-show="tab === 'templates'" class="pane sched-ui">
            <div class="list-table-container">
                ...
                <div class="d-md-none mt-4">
                    <div
                        v-if="!ctx.sched_times || ctx.sched_times.length === 0"
                ...
                >
                    ...
                    <template #window="{ data }">
                        <div>
                            ...
                            <span
                                v-if="Number(data.end_next_day) === 1"
                                class="tag"
                                >+1</span
                            >
                        </div>
                    ...
                    <template #status="{ data }">
                        <span
                            :class="[
                                'pill',
                                Number(data.is_active) === 1 ? 'ok' : 'muted',
                            ]"
                        >
                            {{
                    ...
                    <template #actions="{ data }">
                        <div class="d-flex gap-2">
                            ...
                            />
                            <Button
                                @click="ctx.schedDeleteTemplate(data)"
                                label="Delete"
                                size="small"
                                severity="danger"
                                variant="text"
        ...
        <section v-show="tab === 'userlinks'" class="pane sched-ui">
            ...
            <div class="list-table-container">
                ...
                <XDataTable
                    :value="ctx.sched_userlinks"
                    :columns="userScheduleColumns"
                    :paginator="false"
                    :showIndex="true"
                >
                    ...
                    <template #effFrom="{ data }">
                        <p>{{ data.effective_from || "—" }}</p>
                    </template>
                    <template #effTo="{ data }">
                        <p>{{ data.effective_to || "—" }}</p>
                    </template>
                    <template #note="{ data }">
                        <p>{{ data.schednote || "—" }}</p>
                    </template>
                    ...
                    <template #actions="{ data }">
                        <Button
                            severity="contrast"
                            variant="text"
                            @click="openModalUserSchedEditMode(data)"
                            label="Edit"
                            class="text-primary"
                            size="small"
                        />
                        <Button
                            ...
                            variant="text"
                            class="text-danger"
                            @click="ctx.schedDeleteUserLink(data)"
                            label="Delete"
                            size="small"
                        />
                    </template>
        ...
        <!---Schedules Modal--->
        <Dialog
            v-model:visible="openScheduleModal"
            modal
            @update:visible="
                (value) => {
                    if (!value) {
        ...
        >
            <form class="section-form" @submit.prevent="submitSchedule()">
                ...
                <div class="actions-container">
                    <Button
                        type="submit"
                        :label="ctx.sched_editTId ? 'Update' : 'Create'"
                        severity="info"
                        size="small"
                        icon="pi pi-save"
                    />
        ...
        <!----UserSchedule Modal-->
        <Dialog
            v-model:visible="openUserScheduleModal"
            modal
            :header="ctx.sched_editUId ? 'Edit User Link' : 'New User Link'"
            @update:visible="
                (value) => {
                    if (!value) {
                        console.log(value);
        ...
        >
            <form class="section-form" @submit.prevent="submitUSchedule()">
                ...
                <fieldset>
                    ...
                    />
                    <small v-if="!ctx.sched_uForm.userId" style="color: #a00"
                        >Select a user.</small
                    >
                </fieldset>
                <fieldset>
                ...
                <div class="actions-container mt-4">
                    <Button
                        ...
                        size="small"
                        type="submit"
                        :label="ctx.sched_editUId ? 'Update' : 'Link'"
...
const employeeOptions = [
    ...
    ...ctx.value.employees.map((e) => ({ value: e.id, label: e.username })),
];
