...
// File: resources/js/components/returnscanner.js
...
export default {
    ...
    methods: {
      ...
      viewReturnDetails(item) {
        alert(`Return Details for Serial: ${item.serial_number}\n${item.secondSerial ? 'Second Serial: ' + item.secondSerial + '\n' : ''}Status: ${this.formatStatus(item.status)}\nLocation: ${item.location}\nDate: ${this.formatDate(item.created_at)}`);
      },
      
      // Store dropdown functions
      async fetchStores() {
        try {
          const response = await axios.get(`${API_BASE_URL}/api/returns/stores`, {
      ...
      async fetchReturnHistory() {
        try {
          const response = await axios.get(`${API_BASE_URL}/api/returns/history`, {
            params: { 
      ...
      async checkDualSerial() {
        ...
        try {
          ...
          const response = await axios.get(`${API_BASE_URL}/api/returns/check-serial`, {
          ...
          if (response.data.success) {
            ...
            if (response.data.isDualSerial) {
              ...
              this.secondSerialLabel = response.data.secondSerialLabel || 'Second Serial';
              ...
              this.$refs.scanner.showScanNotice(`Dual serial product detected. Please scan ${this.secondSerialLabel} as well.`);
      ...
      async processScan(scannedCode = null) {
        try {
          ...
          if (scannedCode) {
            ...
            const isLocation = /^L\d{3}[A-G]$/i.test(scannedCode) || 
                            scannedCode === 'Floor' || 
                            scannedCode === 'L800G';
          ...
          const locationRegex = /^L\d{3}[A-G]$/i;
          const isValidLocation = !scanLocation || 
          ...
          // Send to API
          const response = await axios.post(`${API_BASE_URL}/api/returns/process-scan`, scanData, {
            withCredentials: true,
            headers: {
