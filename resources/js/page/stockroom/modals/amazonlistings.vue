    ...
    <div class="p-3 border-bottom-1 surface-border">
      <div class="grid align-items-center">
        ...
        <div class="col-12 md:col-2">
          ...
          <Dropdown
            v-model="filters.identifiersType"
        ...
        <div class="col-12 md:col-5">
          ...
          <InputText
            v-model="filters.identifiersRaw"
            class="w-full"
            placeholder="Search SKU, ASIN, FNSKU, UPC/EAN..."
      ...
      <div class="grid align-items-end mt-2">
        ...
        <div class="col-12 md:col-6 flex justify-content-end gap-2">
          <Button
            ...
            severity="secondary"
            :disabled="loading || !page.prevToken"
            @click="goPrev"
          />
          <Button
            label="Next"
          ...
          <template #body="{ data }">
            <div class="font-medium">
              ...
              >
                <span
                  class="status-dot"
                  :class="data.status === 'ACTIVE' ? 'status-active' : 'status-other'"
                ></span>
                {{ data.status || '—' }}
              </span>
            </div>
...
<script>
import { HttpClient } from "@/helpers/httpClient"; // use your axios wrapper if you have one
...
export default {
  ...
  methods: {
    ...
    async runSearch(resetPaging = false) {
      ...
      this.loading = true;
      try {
        // Use your axios; example:
        const res = await HttpClient.post("/api/amazon/search-listings", payload);
        ...
        console.error("searchListings error:", err?.response?.data || err);
      } finally {
        this.loading = false;
      }
    },
...
<style scoped>
.border-bottom-1 { border-bottom: 1px solid var(--surface-border); }
.border-top-1 { border-top: 1px solid var(--surface-border); }
.truncate { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 620px; }
