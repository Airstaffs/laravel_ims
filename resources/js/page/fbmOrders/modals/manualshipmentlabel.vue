        ...
        <form @submit.prevent="submitLabel" enctype="multipart/form-data">
          ...
          <div class="mb-3">
            ...
            <div v-for="(item, index) in form.OrderItemIds" :key="index" class="input-group mb-2">
              ...
              <button type="button" class="btn btn-danger" @click="removeOrderItemId(index)"
                v-if="form.OrderItemIds.length > 1" style="width: 35px;">X</button>
            </div>
          ...
          <fieldset>
            <label class="form-label d-flex justify-content-between align-items-center">
              <span>Carrier Description <i class="bi bi-info-circle-fill ms-1 text-primary" data-bs-toggle="tooltip"
              ...
              <div class="btn-group btn-group-sm ms-2" role="group">
                <button type="button" class="btn"
                  :class="carrierMode === 'input' ? 'btn-primary' : 'btn-outline-secondary'"
                  @click="carrierMode = 'input'">
                  Input
                </button>
                <button type="button" class="btn"
                  :class="carrierMode === 'select' ? 'btn-primary' : 'btn-outline-secondary'"
            ...
            <select v-else v-model="form.CarrierDescription" class="form-select" required>
              <option disabled value="">-- Select carrier description --</option>
          ...
          <fieldset>
            ...
            <small class="text-danger mt-1 d-block">⚠ Please upload a valid PDF file only. Other file types are not supported.</small>
          </fieldset>
...
export default {
  ...
  methods: {
    toggleCarrierMode() {
      this.carrierMode = this.carrierMode === 'input' ? 'select' : 'input';
    },
    async addCarrierDescription() {
      ...
      try {
        const res = await axios.post(`${API_BASE_URL}/fbm-orders-add-new-carrier`, {
          name: value
        });
        ...
        if (res.data.success) {
          ...
          alert(res.data.error || 'Failed to save carrier description.');
        }
      } catch (err) {
        console.error(err);
    ...
    async fetchCarrierDescriptions() {
      try {
        const res = await axios.get(`${API_BASE_URL}/fbm-orders-carrier-options`);
