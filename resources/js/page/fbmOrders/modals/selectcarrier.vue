  ...
  >
    <div class="carrier-modal" role="dialog" aria-modal="true">
      ...
      <div class="carrier-modal-tabs">
        ...
        >
          Rejected
          <span class="chip chip-bad">{{ rejectedRates.length }}</span>
        </button>
      </div>
      ...
      <div class="carrier-modal-body">
        ...
        <div v-if="tab === 'eligible'">
          ...
          <div v-else class="carrier-list">
            <button
              v-for="(rate, idx) in eligibleRates"
              :key="rate.ShippingServiceId || idx"
              type="button"
              class="carrier-card"
              :class="{ selected: isSelected(rate) }"
              @click="select(rate)"
            >
              ...
              <div class="carrier-meta">
                ...
                <div class="carrier-code">
                  {{ rate.CarrierName }} • {{ rate.ShippingServiceId }}
                </div>
              </div>
        ...
        <div v-else>
          ...
          <div v-else class="rejected-list">
            <div
              v-for="(rej, idx) in rejectedRates"
              :key="rej.ShippingServiceId || idx"
              class="rejected-card"
            >
              <div class="rejected-top">
                <div class="rejected-left">
                  ...
                  <div class="rejected-code">
                    {{ rej.CarrierName }} • {{ rej.ShippingServiceId }}
                  </div>
                </div>
              ...
              <div class="rejected-msg">
                {{ rej.RejectionReasonMessage || "No rejection message provided." }}
              </div>
            </div>
          </div>
        </div>
...
@media (max-width:576px){
  .carrier-modal-backdrop{ padding:0; }
  .carrier-modal{
    width:100%;
    height:100vh;
    max-height:100vh;
...
.carrier-modal-titlewrap{ display:flex; flex-direction:column; gap:2px; }
.carrier-modal-title{
  margin:0;
  font-size:15px;
...
.carrier-close:hover{ background:#f8fafc; }
...
.carrier-tab.active{
  background:#f8fbff;          /* subtle, not solid blue */
  border-color:#c7dbff;
  color:#0b5ed7;
}
...
.chip-ok{ background:#eefbf3; color:#167a3e; border:1px solid #d7f3e2; }
.chip-bad{ background:#fff1f2; color:#b42318; border:1px solid #ffd6db; }
...
.carrier-card:active{ transform:none; } /* remove “press” effect for minimalism */
...
.footer-btn:active{ transform:none; }
