...
class SwitcheruController extends BasetablesController
{
    public function index(Request $request)
    {
        try {
            ...
            // ── Check if tblswitcherus exists ──
            if (!Schema::hasTable('tblswitcherus')) {
                Log::error('tblswitcherus table does not exist');
                return response()->json([
                    'data' => [],
                    'total' => 0,
                    'error' => 'tblswitcherus table not found',
                ]);
            }

            // ── Main query on tblswitcherus ──
            $query = DB::table('tblswitcherus as sw')
                ->select(['sw.*']);
            ...
            // ── Attach captured images for both sent & received serials ──
            try {
                $capturedImagesTable = $this->capturedImagesTable;
                $productTable = $this->productTable;
                $company = $this->company;
                ...
                Log::info('Captured images table check', [
                    'table' => $capturedImagesTable,
                    'exists' => $hasCapturedImagesTable,
                ]);
                ...
                if (count($allSerials) && Schema::hasTable($productTable)) {
                    // Find products matching these serials
                    $productRows = DB::table($productTable)
                        ...
                        ->select(['ProductID', 'serialnumber', 'serialnumberb', 'img1', 'BuyerName', 'costumer_name', 'FNSKUviewer'])
                        ->get();
                ...
                $results->getCollection()->transform(function ($row) use ($serialToProduct, $capturedImages, $company) {
                    ...
                    // Fallback buyer
                    if (empty($row->buyer) || $row->buyer === 'Unknown') {
                        $p = $row->sentProduct ?? $row->receivedProduct;
                        if ($p) {
                            $row->buyer = $p->BuyerName ?: ($p->costumer_name ?: 'Unknown');
                        }
                    }
